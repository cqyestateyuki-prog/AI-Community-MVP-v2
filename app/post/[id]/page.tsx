...
import { 
  ...
  ThumbsUp, ExternalLink, Calendar, MapPin, Code, MessageSquare, Star, Trash2
} from 'lucide-react';
...
export default function PostDetailPage() {
  ...
  const handleDeletePost = () => {
    if (!post) return;
    const success = storage.deletePost(post.id);
    if (success) {
      router.push('/');
    }
  };
  ...
  // 检查是否是当前用户的帖子
  const isCurrentUserPost = post && post.author.name === storage.getCurrentUser().name;
        ...
        <div className="flex items-center justify-between mb-6">
          ...
          {isCurrentUserPost && (
            <button
              ...
              className="flex items-center gap-2 text-red-500 hover:text-red-700 hover:bg-red-50 px-3 py-1 rounded transition-colors"
              title="Delete post"
            >
